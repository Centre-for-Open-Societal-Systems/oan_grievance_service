...
**Unified routing and access scope on Grievance RBAC Assignment.** Routing and access are consolidated under `Grievance RBAC Assignment`, supporting `category_scope`, `grievance_type_scope`, `service_provider_scope`, and `administrative_area_scope`. Precedence is resolved automatically through Nearest-Ancestor administrative area tree containment and scope specificity without manual precedence numbers. All matching desks assign the department and route officers according to the configured `routing_strategy` (primary_first / round_robin / least_loaded).
